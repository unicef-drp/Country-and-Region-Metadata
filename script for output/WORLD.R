...
dir.create("raw_data/WORLD", showWarnings = FALSE, recursive = TRUE)
dest <- file.path("raw_data/WORLD/WPP2024_TotalPopulationBySex.csv.gz")
download.file(url, destfile = dest, mode = "wb")

dt_in <- fread(dest)
setDT(dt_in)

dt_in <- dt_in[, list(ISO3_code, LocID, SDMX_code, Location)]
...
# SDMX name recode map (none currently needed for WPP)
SDMX_recode <- c()
...
# SOWC World (moved from UNICEF_NT)
